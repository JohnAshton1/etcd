...
func (m *Monkeys) CrushPods(ctx context.Context, c *CrashConfig) {
	...
	for {
		...
		pods, err := m.kubecli.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{LabelSelector: ls})
		if err != nil {
		...
		for tokill := range tokills {
			err = m.kubecli.CoreV1().Pods(ns).Delete(ctx, tokill, metav1.NewDeleteOptions(0))
			if err != nil {
